...
const AstronomyPillar = lazy(() => import('./components/AstronomyPillarLive').then(m => ({ default: m.AstronomyPillarLive || m.AstronomyPillar || m.default })));
...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-transparent text-slate-100">
      ...
      <div className="border-b border-slate-800/80 bg-slate-950/40">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-1.5 flex items-center justify-between gap-2 text-[11px] text-slate-400">
          <span className="flex items-center gap-1.5 truncate">
            ...
            {companyContent?.tagline || 'Global time, calendars & tools'}
          </span>
          <button
            type="button"
        ...
        <div className="flex-1 min-w-0">
          <PillarErrorBoundary>
            <Suspense fallback={<PillarLoader pillarNumber={activePillar} isDarkMode={isDarkMode} />}>
              ...
              {activePillar === 2 && (
                <PillarChrome pillarId={2}><CalendarPillar isDarkMode={isDarkMode} /></PillarChrome>
              )}
              {activePillar === 3 && (
              ...
              {activePillar === 5 && (
                <PillarChrome pillarId={5}><TimersPillar isDarkMode={isDarkMode} /></PillarChrome>
              )}
              {activePillar === 6 && (
                <PillarChrome pillarId={6}><WorldometersPillar isDarkMode={isDarkMode} /></PillarChrome>
              )}
              {activePillar === 7 && (
                <PillarChrome pillarId={7}><WidgetsPillar isDarkMode={isDarkMode} /></PillarChrome>
              )}
              {activePillar === 8 && (
                <PillarChrome pillarId={8}><EnterpriseServicesPillar /></PillarChrome>
              )}
              {activePillar === 9 && (
                <PillarChrome pillarId={9}><NewsPillar isDarkMode={isDarkMode} /></PillarChrome>
              )}
              {activePillar === 10 && (
                <PillarChrome pillarId={10}><CalculatorsPillar isDarkMode={isDarkMode} /></PillarChrome>
              )}
              {activePillar === 11 && (
      ...
      {shortcutToast && <ShortcutToast message={shortcutToast} onDone={() => setShortcutToast(null)} />}
    </div>
  );
}
