...
import { submitExperienceFeedback, feedbackMailto, type ExperienceVote } from '../lib/experienceFeedback';
...
export const SiteFooter: React.FC<Props> = ({
  ...
  const submit = () => {
    const entry = submitExperienceFeedback({
      vote: pendingVote,
      comment,
      displayName,
      wantPublic,
    ...
    setDoneMsg(
      wantPublic
        ? 'Thanks — saved. Public share waits for site owner approval (Company → Feedback).'
  ...
  const linkBtn =
    'text-left text-[13px] leading-relaxed text-slate-600 dark:text-slate-300 hover:text-cyan-600 dark:hover:text-cyan-400 hover:underline font-medium';
  const colTitle =
    'text-[12px] font-bold uppercase tracking-wide text-slate-800 dark:text-slate-100 mb-2.5';

  return (
    <footer className="w-full mt-auto font-sans text-slate-700 dark:text-slate-300">
      ...
      {showForm && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/50" role="dialog">
          <div className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-2xl p-5 space-y-3">
            ...
            <label className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-400 cursor-pointer">
              ...
              <span>I would like this comment shown on the public Feedback wall. It only appears after the site owner approves it.</span>
            </label>
            <div className="flex gap-2 justify-end">
              <button type="button" onClick={() => setShowForm(false)} className="px-3 py-2 text-xs font-semibold text-slate-500">Cancel</button>
      ...
      <div className={isDarkMode ? 'bg-slate-900 border-t border-slate-800' : 'bg-slate-100 border-t border-slate-200'}>
        <div className="max-w-[1200px] mx-auto px-4 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-6">
            <div className="lg:col-span-4 flex gap-4">
              ...
              <div className="min-w-0">
                ...
                <ul className="space-y-1.5 text-[13px] text-slate-600 dark:text-slate-300 list-disc pl-4">
                  <li>Browse our site <strong className="text-slate-900 dark:text-white">advert free</strong>.</li>
                  <li>Sun &amp; Moon times <strong className="text-slate-900 dark:text-white">precise to the second</strong>.</li>
                  <li><strong className="text-slate-900 dark:text-white">Exclusive calendar templates</strong> for PDF Calendar.</li>
                </ul>
                ...
                <p className="mt-2 text-[11px] text-slate-500 dark:text-slate-400">From <strong>A$29.99/year</strong> · Not a tax-deductible donation</p>
              </div>
            </div>
            ...
            <div className="lg:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-6">
              <div>
                ...
                <ul className="space-y-1.5">
                  ...
                  <li><button type="button" className={linkBtn} onClick={() => goCompany('contact')}>Contact Details</button></li>
                  ...
                  {(
                    ...
                    href ? (
                      ...
                      <span key={label} title={`${label} — add URL in companyContent`}
                        className="w-8 h-8 rounded-lg bg-slate-300 dark:bg-slate-700 text-slate-500 flex items-center justify-center">
                        <Icon className="w-3.5 h-3.5" />
                      </span>
                    )
                  )}
          ...
          <div className="mt-8 pt-6 border-t border-slate-300 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-2">
              <Globe className="w-5 h-5 text-cyan-600 dark:text-cyan-400" />
              <span className="font-extrabold text-slate-800 dark:text-white text-sm">
                {c.brandName}<span className="text-cyan-600 dark:text-cyan-400">.com</span>
              </span>
            </div>
