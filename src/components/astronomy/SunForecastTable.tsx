...
import { getMemberEntitlements } from '../../lib/memberEntitlements';
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 overflow-hidden shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-3 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center gap-2 text-sm font-bold text-slate-800 dark:text-slate-100 flex-wrap">
          ...
          {precise ? (
            <span className="text-[9px] font-bold uppercase tracking-wide text-emerald-600 dark:text-emerald-400 px-1.5 py-0.5 rounded bg-emerald-500/10">
              to the second
            </span>
          ) : (
            <span className="text-[9px] text-slate-400" title="Become a Supporter for times to the second">
              HH:mm · Supporter unlocks seconds
            </span>
          )}
        </div>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {rows.map((r) => (
              ...
              >
                ...
                <td className="px-3 py-2 font-mono">{formatLocalTime(r.sunrise, precise)}</td>
                <td className="px-3 py-2 font-mono text-slate-500">{formatLocalTime(r.solarNoon, precise)}</td>
                <td className="px-3 py-2 font-mono">{formatLocalTime(r.sunset, precise)}</td>
      ...
      <p className="px-4 py-2 text-[10px] text-slate-400 border-t border-slate-100 dark:border-slate-800">
        Computed client-side. Free: HH:mm · Supporter: times to the second.
      </p>
    </div>
  );
};
