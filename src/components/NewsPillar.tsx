...
export const NewsPillar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const onVis = () => {
      if (document.visibilityState === 'visible') fetchNewsFeed({ category: selectedCategory, topic: activeTopic });
    };
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm text-slate-900 dark:text-slate-100 space-y-5">
        <div className="flex flex-col gap-4">
          ...
          <form onSubmit={handleCustomTopicSearch} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full">
            ...
            <button type="submit" disabled={isRefreshing || !customTopicInput.trim()} className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-semibold text-xs rounded-xl transition-all cursor-pointer whitespace-nowrap flex items-center justify-center gap-1.5 shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-1.5 text-xs font-semibold pt-2">
          {[{ id: 'all', label: 'All News' }, { id: 'world', label: 'World' }, { id: 'astronomy', label: 'Astronomy' }, { id: 'timezones', label: 'Time Zones' }, { id: 'dst', label: 'DST' }, { id: 'leap_seconds', label: 'Leap Seconds' }, { id: 'technology', label: 'Tech' }].map((cat) => (
            <button key={cat.id} type="button" onClick={() => handleCategoryChange(cat.id)} className={`px-3 py-1.5 rounded-xl transition-all cursor-pointer ${selectedCategory === cat.id ? 'bg-blue-600 text-white shadow-xs' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
              {cat.label}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredArticles.map((article) => {
          ...
          return (
            <article key={article.id} role="link" tabIndex={0} onClick={openArticle} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openArticle(); } }} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col justify-between group cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500">
              <div>
                <div className="relative h-44 sm:h-52 overflow-hidden bg-gradient-to-br from-slate-800 via-indigo-950 to-slate-900">
                  ...
                  <div className="absolute top-3 right-3 z-10">
                    <span className="bg-slate-900/80 backdrop-blur-md text-cyan-300 text-[10px] font-mono px-2 py-0.5 rounded border border-cyan-500/30">{article.timeAgo}</span>
                  </div>
              ...
              <div className="px-5 pb-5 pt-2 flex items-center justify-between border-t border-slate-100 dark:border-slate-800/80 text-xs">
                <button type="button" onClick={(e) => { e.stopPropagation(); copyArticleCitation(article); }} className="text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 text-[11px] font-medium flex items-center gap-1 cursor-pointer">
                ...
                <div className="flex items-center gap-3">
                  {article.sourceUrl && (
                    <a href={article.sourceUrl} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="text-slate-500 hover:text-blue-600 dark:hover:text-cyan-400 font-medium flex items-center gap-1 text-[11px]">
                      <span>Source</span>
      ...
      {activeArticle && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-3xl w-full p-6 space-y-5 relative max-h-[90vh] overflow-y-auto">
            <button type="button" onClick={() => setActiveArticle(null)} className="absolute top-4 right-4 text-slate-400 p-2">✕</button>
            ...
            {activeArticle.sourceUrl && (
              <a href={activeArticle.sourceUrl} target="_blank" rel="noopener noreferrer" className="inline-block px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-xl">Open Source</a>
            )}
          </div>
        </div>
      )}
