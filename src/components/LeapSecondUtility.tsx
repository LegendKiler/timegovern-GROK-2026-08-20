...
export const LeapSecondUtility: React.FC<LeapSecondUtilityProps> = ({ compact = false }) => {
    ...
    if (!notificationEnabled) {
      ...
      try {
        ...
        if (perm === 'granted') {
          ...
          try {
            new Notification('ÔÅ¦´©Å Timegovern Leap Second Alert Armed', {
  ...
  const handleTriggerTestAlert = () => {
    ...
    const alertTitle = 'ÔÜá´©Å Timegovern Leap Second Alert (T-24h)';
  ...
  const SIM_STEPS = {
    standard: [
      ...
      { time: '23:59:60.000', label: 'LEAP SECOND (+1s)', desc: '­ƒîƒ The inserted 61st second! TAI continues unbroken.' },
    ...
    negative: [
      ...
      { time: '00:00:00.000', label: 'SKIPPED 23:59:59 (-1s)', desc: 'ÔÜá´©Å 23:59:59 is omitted entirely! Clock jumps directly to midnight' },
          ...
          <div className="flex items-start gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-2xl leading-relaxed">
                Real-time telemetry and synchronization confidence derived from authoritative reference feeds (IANA tzdb, IERS Paris, BIPM S+¿vres, NIST Cesium Fountains, and NIST time services).
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-extrabold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
              ...
              Complete Historical Leap Seconds Registry (1972ÔÇô2026)
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
        <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-xl">
          ...
          <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
            Earth's rotational velocity is affected by lunar tidal friction, core-mantle fluid momentum transfer, atmospheric jet streams, and post-glacial isostatic rebound. While long-term trends slow Earth down, short-term anomalies in 2020ÔÇô2026 showed slight acceleration.
          </p>
        </div>
